...
const PopularHotels = () => {
  const { data, isLoading, isError, refetch } = useGetHotelsQuery("goa");
  const hotels = data?.data || [];
  console.log(hotels);
  const polularHotels = [...hotels].sort((a, b) => b.starRating - a.starRating);
  console.log(polularHotels);
  // --Loading..
  // if (isLoading) {
  //   return (
  //     <div className="flex gap-5 overflow-x-auto py-10">
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex items-end justify-between py-5">
        ...
        <Link to={`hotelLists`}>View All</Link>
      </div>
      {isLoading ? (
