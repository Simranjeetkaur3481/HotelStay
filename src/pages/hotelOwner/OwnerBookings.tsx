...
const OwnerBookings = () => {
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Bookings" description="View and manage all reservations across your properties." />
      ...
      {isLoading ? (
        ...
        <EmptyState icon={CalendarDays} title="No bookings yet" description="Bookings will appear here once guests reserve your rooms." />
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  <th className="px-6 py-4 text-left font-medium">Guest / Hotel</th>
                ...
                {bookings.map((booking: {
                  ...
                  <tr key={booking.id} className="border-b transition hover:bg-muted/20">
                    <td className="px-6 py-4">
                      <p className="font-medium">{booking.guestName ?? `Booking #${booking.id}`}</p>
